...
static int check_block_sizes(size_t fixed_block_frames, size_t fixed_input_block_frames, size_t overlap_frames, pa_vsink *vs) {
    size_t max_block_frames;
    size_t max_frame_size;

    max_frame_size = PA_MAX(pa_frame_size(&vs->sink->sample_spec), pa_frame_size(&vs->input_to_master->sample_spec));

    max_block_frames = pa_mempool_block_size_max(vs->sink->core->mempool);
...
static void set_latency_range_within_thread(pa_vsink *vsink) {
    ...
    pa_sink_input *i = vsink->input_to_master;
    pa_sink *s = vsink->sink;
    ...
    switch (code) {
        ...
        case PA_SINK_MESSAGE_GET_LATENCY:
           ...
           if (vsink->input_to_master->thread_info.resampler)
               *((int64_t*) data) += pa_resampler_get_delay_usec(vsink->input_to_master->thread_info.resampler);
        ...
        if (vsink->update_filter_parameters) {
            ...
            parameters = vsink->update_filter_parameters(data, s->userdata);
            if (parameters)
                pa_asyncmsgq_post(pa_thread_mq_get()->outq, PA_MSGOBJECT(vsink), VSINK_MESSAGE_FREE_PARAMETERS, parameters, 0, NULL, NULL);

            /* Updating the parameters may have changed the block sizes, so check them again. */
            if (check_block_sizes(vsink->fixed_block_size, vsink->fixed_input_block_size, vsink->overlap_frames, vsink) < 0) {
...
int pa_virtual_sink_set_state_in_io_thread(pa_sink *s, pa_sink_state_t new_state, pa_suspend_cause_t new_suspend_cause) {
    ...
    /* When set to running, request a rewind of the master sink to make sure
     * we are heard immediately. Also set max_rewind on sink and master sink */
    if (PA_SINK_IS_RUNNING(new_state) && !PA_SINK_IS_RUNNING(s->thread_info.state)) {
...
void pa_virtual_sink_update_requested_latency(pa_sink *s) {
     ...
     * in the virtual sink. Reduce master latency by this amount. Do not set
    ...
    if (vsink->fixed_block_size) {
        ...
        in_fs = pa_frame_size(&s->sample_spec);
        fixed_block_latency = pa_bytes_to_usec(vsink->fixed_block_size * in_fs, &s->sample_spec);
        min_latency = i->sink->thread_info.min_latency;
    ...
    /* Now hand this one over to the master sink */
...
int pa_virtual_sink_input_pop(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    ...
    /* Process any rewind request that might be queued up. */
    pa_sink_process_rewind(s, 0);

    if (!vsink->process_chunk || !vsink->memblockq) {
    ...
    out_fs = pa_frame_size(&i->sample_spec);
    in_fs = pa_frame_size(&s->sample_spec);
    max_block_frames = pa_mempool_block_size_max(i->sink->core->mempool) / PA_MAX(in_fs, out_fs);
    nbytes = PA_MIN(nbytes, max_block_frames * out_fs);
    ...
    /* Release result chunk, the data will be fetched again later. */
    pa_memblock_unref(tchunk.memblock);
    pa_memchunk_reset(&tchunk);
    ...
    /* If fixed input block size is used, the overlap_framse value will be ignored. */
    ...
    /* Now rewind the memblockq to get some history data if required. */
    if (overlap_frames)
        pa_memblockq_rewind(vsink->memblockq, overlap_frames * in_fs);
    ...
    pa_memblockq_peek_fixed_size(vsink->memblockq, in_count * in_fs, &tchunk);
...
void pa_virtual_sink_input_process_rewind(pa_sink_input *i, size_t nbytes) {
    ...
    if (vsink->memblockq)
        ...
        pa_sink_process_rewind(s, nbytes * in_fs / out_fs);
...
void pa_virtual_sink_input_kill(pa_sink_input *i) {
     ...
     * to the master. It may be possible that the sink input is connected
     * to a virtual sink which has lost its master, so do not try to cork
     * if the sink has no I/O context. */
    if (i->sink && i->sink->asyncmsgq)
...
static int vsink_process_msg(pa_msgobject *o, int code, void *userdata, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case VSINK_MESSAGE_FREE_PARAMETERS:

            pa_assert(userdata);
            pa_assert(vsink->free_filter_parameters);
            vsink->free_filter_parameters(userdata);
            return 0;
...
int pa_virtual_sink_activate(pa_vsink *vs) {
    ...
    /* If volume sharing and flat volumes are disabled, we have to apply the sink volume to the sink input. */
    if (!(vs->sink->flags & PA_SINK_SHARE_VOLUME_WITH_MASTER) && !pa_sink_flat_volume_enabled(vs->input_to_master->sink)) {
        pa_cvolume vol;

        vol = vs->sink->real_volume;
        pa_cvolume_remap(&vol, &vs->sink->channel_map, &vs->input_to_master->channel_map);
        pa_sink_input_set_volume(vs->input_to_master, &vol, vs->sink->save_volume, true);
    }
...
void pa_virtual_sink_destroy(pa_vsink *vs) {
     ...
     * VSINK_MESSAGE_INPUT_ATTACHED messages. */
...
pa_vsink *pa_virtual_sink_create(pa_sink *master, const char *sink_type, const char *desc_prefix,
    ...
    if (!pa_safe_streq(master->name, m->core->default_sink->name))
        sink_input_data.preferred_sink = pa_xstrdup(master->name);
